...
while ~isempty(openList)
    ...
    if size(path_show,1)>=4
        [~,~,path_show] = Astar.calFitness(X,Y,Z, path_show,intrp_num);
    ...
    for i = 1:length(neighbors)
        ...
        % 计算邻居节点的g值
        neighbor_path=[Astar.reconstructPath(currentNode);neighbor.pos];
        if size(neighbor_path,1)>=4
            [flag,fitness,~] = Astar.calFitness(X,Y,Z, neighbor_path,intrp_num);        
            if flag == 0 
                tentative_g = fitness;
            else
                tentative_g = 1000* fitness;
            end
        else
            [flag,fitness,~] = Astar.calH(X,Y,Z, neighbor_path);        
...
GlobalBest.path=path;
disp(['得到的Fitness ' num2str(fitness)]);
Astar.plotFigure(startPos,goalPos,X,Y,Z,path_show);
toc
savefig("exp/Astar_final_fig") 
save("exp/Astar_GlobalBest.mat","GlobalBest")
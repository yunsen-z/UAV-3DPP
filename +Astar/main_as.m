...
while ~isempty(openList)
    ...
    if size(path_show,1)>=4
        [~,~,path_show] = calFitness(X,Y,Z, path_show,intrp_num);
    ...
    for i = 1:length(neighbors)
        ...
        % 计算邻居节点的g值
        neighbor_path=[reconstructPath(currentNode);neighbor.pos];
        if size(neighbor_path,1)>=4
            [flag,fitness,~] = calFitness(X,Y,Z, neighbor_path,intrp_num);        
            if flag == 0 
                tentative_g = fitness;
            else
            ...
            [flag,fitness,~] = calH(X,Y,Z, neighbor_path);        
